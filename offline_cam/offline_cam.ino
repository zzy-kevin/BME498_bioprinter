...
void setup() {
  ...
  config.frame_size = FRAMESIZE_UXGA; // adjustable
  config.pixel_format = PIXFORMAT_JPEG;  // for streaming
  config.grab_mode = CAMERA_GRAB_WHEN_EMPTY;
  config.fb_location = CAMERA_FB_IN_PSRAM;
  config.jpeg_quality = 12; // adjustable smaller - less compression & better quality
  config.fb_count = 1;
...
void loop() {
    ...
    if (!fb) {
        ...
        while (true);  // Stop execution if capture fails
    }
    ...
    Serial.write(fb->buf, fb->len);  // Send binary image data
    Serial.println(); // add new line for end maker
    Serial.println("<END>");     // Send end marker
    Serial.println("Waiting for ACK..."); // wait python receiving image

    // 🔹 Wait for Python to confirm reception
    while (true) {
        if (Serial.available()) {
            String response = Serial.readStringUntil('\n');
            response.trim();
            if (response == "ACK") {
                Serial.println("ACK received! Freeing memory...");
                break;
            }
        }
    }
